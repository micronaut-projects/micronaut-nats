...
snippet::io.micronaut.nats.docs.headers.ProductClient[tags="imports,clazz", indent=0, project-base="docs-examples/example"]
...
[.lang-python]
NOTE: Python methods cannot be overloaded, so the Python client declares one method per signature (`send`, `send_with_headers`, `send_with_product_sizes`, `send_with_nats_headers`).
