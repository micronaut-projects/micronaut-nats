...
[.lang-python]
<1> Defining a queue is done in the ann:configuration.nats.annotation.Subject[]; the `queue` member cannot be set on a Python method yet (Python compiler limitation), so both Python methods above subscribe without a queue group.
